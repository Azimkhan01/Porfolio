...
function DownloadResume() {
  ...
  // Detect outside clicks (works on mobile + desktop)
  useEffect(() => {
  ...
  return (
    <div
      ...
      tabIndex={0}
      className={`${
        isClick ? "w-40 h-50" : "w-20 h-20"
      } flex flex-col justify-evenly items-center transition-all ease-out duration-200 z-5 fixed bottom-5 rounded-3xl border-1 border-white right-5 bg-[#000000df]`}
    >
      <div
        className={`${
          !isClick
      ...
      <button
        className={`${
          isClick
            ? "h-12 p-3 mb-5 rotate-45 text-white bg-black border-1 border-[gray]"
            : "bg-white"
        } h-1/2 text-2xl font-light p-2 rounded-full flex justify-center items-center transition-all ease-in-out duration-300`}
