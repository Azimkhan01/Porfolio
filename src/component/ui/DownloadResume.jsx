...
function DownloadResume() {
  ...
  return (
    <div tableIndex={0} onBlur={()=>{setIsClick(!isClick)}}  className={`${isClick ? 'w-40 h-50' : ' w-20 h-20' } flex flex-col  justify-evenly  items-center transition-all ease-out duration-200  z-5 fixed bottom-5 rounded-3xl border-1 border-white right-5  bg-[#000000df]`}>
        <div  className={`${!isClick ? "hidden" : "flex flex-col gap-5 items-center justify-between"} `}>
            ...
            <a href="https://drive.google.com/file/d/1kuMOqnZNRJ70fNgLYMXpE-zt9YTFvuCr/view?usp=drive_link"><AiOutlineDownload className='text-white text-2xl active:scale-150  hover:scale-150 transition-all ease-linear duration-200' /></a>
        </div>
